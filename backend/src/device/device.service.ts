...
export class DeviceService {
  ...
  async addDeviceToGateway(addDeviceToGatewayDTO: AddDeviceToGatewayDTO) :Promise<boolean> {
    const { deviceId , gatewayId } = addDeviceToGatewayDTO;
    const devicesCount = await this.getGatewayDevicesCount(gatewayId);
    if(devicesCount >= 10){
      throw new CustomError(Constants.API_RESPONSE_MESSAGES.LIMIT_EXCEEDED_MESSAGE,400,Constants.API_RESPONSE_MESSAGES.LIMIT_EXCEEDED);
    }
    ...
    if(device){
      throw new CustomError(`Device #${deviceId} is already connected to gateway #${gatewayId}`,400,'Already Found');
    }
    ...
    if (!deviceUpdated) {
        throw new NotFoundException(Constants.API_RESPONSE_MESSAGES.ERROR_ASSINING_DEVICE);
    }
    return true;
  }
  ...
  async updateDevice(deviceId: string, updateDeviceDto: UpdateDeviceDto) : Promise<IDevice> {
    const device = await this.deviceModel.findByIdAndUpdate(deviceId, updateDeviceDto, { new: true });
    if (!device) {
      throw new NotFoundException(`Device #${deviceId} not found`);
    }
    return device;
  ...
  async removeDeviceFromGateway(gatewayId:string, deviceId: string): Promise<any> {
    const device = await this.deviceModel.findOneAndUpdate({
      gateway: mongoose.Types.ObjectId(gatewayId),
      _id: mongoose.Types.ObjectId(deviceId)
    },{ $set: { gateway: null } });
    if (!device) {
        throw new NotFoundException(`Device #${deviceId} not found in gateway #${gatewayId}`);
    }
  ...
  async generateNewDeviceId() :Promise<number> {
    const lastDevice = await this.deviceModel.findOne().sort({ uid: 'desc' }).limit(1);
    let newID = lastDevice ? lastDevice.uid + 1 : 1; 
    return newID;
  }

  async getGatewayDevicesCount(gatewayId :string) :Promise<number> {
    return await this.deviceModel.countDocuments({ gateway: mongoose.Types.ObjectId(gatewayId) });
  }

  async removeGatewayFromDevices(gatewayId :string): Promise<any> {
    return await this.deviceModel.updateMany({ gateway: mongoose.Types.ObjectId(gatewayId) },{ $set: { gateway: null } });
  }
